...
class MainPage(RenderableLivePage):
    ...
    def handleImportPDF(self, client, path, importString):
        ...
        self.outlinePane.handleAddChild(client,
            self.package.currentNode.id)
        self.outlinePane.handleRenNode(client, self.package.currentNode.id,
            pageName)
        root = self.package.currentNode
        ...
        for x in toimport:
            ...
            # number manually
            self.package.currentNode.idevices[0].pages = str(x + 1)
            self.package.currentNode.idevices[0].uploadFile()
            self.package.currentNode = root
        client.sendScript((u'top.location = "/%s"' % self.package.name).encode('utf8'))
    ...
    def exportPresentation(self, client, filename, stylesDir):
        ...
        try:
            ...
            presentationExport = PresentationExport(self.config, stylesDir,
                filename)
            presentationExport.export(self.package)
        except Exception, e:
