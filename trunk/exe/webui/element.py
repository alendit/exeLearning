...
class TermElement(Element):
    """
    TermElement is responsible for term definition
    Used by Glossary iDevice
    """
    ...
    def __init__(self, field):
        ...
        self.termElement.id = self.termId
        self.definitionElement = TextAreaElement(field.definitionArea)
        self.definitionElement.height = 250
        self.definitionId = "definition" + self.id
        self.definitionElement.id = self.definitionId

    def process(self, request):
        """
        Process the request arguments from web server
        """
        log.info("process " + repr(request.args))

        is_cancel = common.requestHasCancel(request)

        if self.termId in request.args:
            self.termElement.process(request)
        if self.definitionId in request.args:
            self.definitionElement.process(request)
        ...
        if "action" in request.args and \
           request.args["action"][0] == "del" + self.id:
            ...
            self.field.idevice.terms.remove(self.field)
            self.field.idevice.undo = False

    def renderEdit(self):
        """
        Returns and XHTML string with the form element for editing this element
        """
        ...
        html += u" " + common.submitImage("del" + self.id, 
                                   ...
                                   _("Delete question"))
        # rather than using termElement.renderEdit(),
        ...
        this_package = None
        if self.termElement.field_idevice is not None \
        and self.termElement.field_idevice.parentNode is not None:
            this_package = self.termElement.field_idevice.parentNode.package

        html += common.richTextArea("term" + self.id, 
                       self.termElement.field.content_w_resourcePaths,
                       package=this_package)

        html += self.definitionElement.renderEdit()
        html += u"</div>\n"
    ...
    def doRender(self, img, preview=False):
        ...
        if preview:
            #html = self.termElement.renderPreview()
            #html += self.definitionElement.renderPreview()
            visible = 'style="display:block"'
            self.termElement.field.content = \
                self.termElement.field.content_w_resourcePaths
            termContent = self.termElement.field.content
            self.definitionElement.field.content = \
                self.definitionElement.field.content_w_resourcePaths
            ...
            html = self.termElement.renderView()
            html += self.definitionElement.renderView()
            visible = 'style="display:block"'
            self.termElement.field.content = \
                self.termElement.field.content_wo_resourcePaths
            termContent = self.termElement.field.content
            self.definitionElement.field.content = \
